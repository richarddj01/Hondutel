            ...
            <form method="GET" action="{{ route('averias.finalizadas') }}" class="mb-3">
                <div class="row">
                    <div class="col">
                        ...
                        <input type="date" id="fecha_inicio" name="fecha_inicio" class="form-control" value="{{ old('fecha_inicio', request('fecha_inicio')) }}">
                    </div>
                    <div class="col">
                        <label for="fecha_fin" class="form-label">Fecha de fin:</label>
                        <input type="date" id="fecha_fin" name="fecha_fin" class="form-control" value="{{ old('fecha_inicio', request('fecha_fin')) }}">
                    </div>
                    <div class="col mt-4">
                        <button type="submit" class="btn btn-primary">Filtrar</button>
                        <!-- Botón para descargar el PDF -->
                        <a href="{{ route('report.averiasFinalizadas') }}" class="btn btn-info"> <i class="bi bi-file-earmark-pdf-fill"></i> Descargar Reporte PDF</a>
                    </div>
                </div>
            </form>
            ...
            <table class="table">
                ...
                @foreach ($averias as $averia)
                    <tr>
                        ...
                        <td>{{ $averia->updated_at->format('h:i A') }}</td>
                        <td>{{ $averia->numero}}</td>
